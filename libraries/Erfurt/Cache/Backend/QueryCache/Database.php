...
class Erfurt_Cache_Backend_QueryCache_Database extends Erfurt_Cache_Backend_QueryCache_Backend {
	...
	final public function createCacheStructure() {
        ...
        /*adaptations for ArchiMind features by Klaas Andries de Graaf <https://github.com/kadevgraaf/> - 27-07-2016 
        */
        $existingTableNames = $this->store->listTables();

        if (!in_array('ef_experiment', $existingTableNames)) {
            $columnSpec = array(
				  'ID' => 'INT(11) NOT NULL PRIMARY KEY auto_increment',
				  'Name' => 'VARCHAR(100) NOT NULL ',
				  'IP' => 'VARCHAR(100) NOT NULL ',
				  'browser ID' => 'VARCHAR(100) NOT NULL ',
				  'Login time' => 'TIMESTAMP NOT NULL default CURRENT_TIMESTAMP on update CURRENT_TIMESTAMP',
				  'Email' => 'VARCHAR(100) NOT NULL ',
				  'Answer' => 'TEXT NOT NULL ',
				  'Experience' => 'VARCHAR(100) NOT NULL ',
				  'Question' => 'INT(10) NOT NULL ',
				  'Likert' => 'INT(10) NOT NULL '
            );
            
            $this->store->createTable('ef_experiment', $columnSpec);
        }        
        ...
        if (!in_array('ef_annotation', $existingTableNames)) {
            $columnSpec = array(
                'instanceURL'        => 'VARCHAR(220)',
                'phrase'        => 'VARCHAR(220)',
                'conceptClass'        => 'VARCHAR(220)'
            );
            
            $this->store->createTable('ef_annotation', $columnSpec);
        }        
        
        $existingTableNames = $this->store->listTables();

        if (!in_array('ef_diaglog', $existingTableNames)) {
            $columnSpec = array(
                ...
                'object_property'        => 'VARCHAR(250)',
                'data_property'        => 'VARCHAR(250)',
                'user'        => 'VARCHAR(250)'
            );
            
            $this->store->createTable('ef_diaglog', $columnSpec);
        }        
        /*end of adaptations for ArchiMind features by Klaas Andries de Graaf <https://github.com/kadevgraaf/> - 27-07-2016 */
...
/*                   "(
    ...
    private function createNewView ($tableName) {
	 ...
	 CREATE TABLE IF NOT EXISTS `".$tableName."` (
	   ...
	   `o` TEXT(65000) character set utf8 collate utf8_bin NOT NULL,
	   ...
	   PRIMARY KEY  (`id`),
	   UNIQUE KEY `unique_stmt` (`g`,`s`,`p`,`o`(160),`st`,`ot`,`ol`,`od`),
	   KEY `idx_g_p_o_ot` (`g`,`p`,`o` (160),`ot`),
	   KEY `idx_g_o_ot` (`g`,`o`(160),`ot`)
