...
public function addMultipleStatements($graphUri, array $statementsArray, array $options = array())
    {
        ...
        $insertArray = array();
        $counter = 0;
        $wikipage = false; //indicates if wikipage information is updated
        $otherSubject = false;  //check if not multiple instances are created/updated
        $wikipageCounter = 0;
        $wikipageArray = array(); //holds all wikipage instances
        $released = false; //indicates if the file is released
        $exists = true; //assume another wikipage exists
        foreach ($statementsArray as $subject => $predicatesArray) {
            foreach ($predicatesArray as $predicate => $objectsArray) {
                foreach ($objectsArray as $object) {
                    ...
                    //is wikipage when type is wikipage
                    if((strcmp($p, "http://www.w3.org/1999/02/22-rdf-syntax-ns#type") == 0) && (strcmp($oValue, "http://www.semanticweb.org/ontologies/2010/10/Ontology1288790966584.owl#WikiPage") == 0))  {
                        $wikipage = true;
                        $wikipageArray[] = $s;
                        $wikipageCounter++;
                    }
                    //has a label when the label property exists
                    if(strcmp($p, "http://www.w3.org/2000/01/rdf-schema#label") == 0)  {
                        $label = true;
                    }
                    //if the released option exists set it to N
                    if(strcmp($p, 'http://localhost/ontowiki/view/released') == 0)  {
                        $released = true;
                    }
                    if($otherSubject == false)  {
                        $otherSubject = $s;
                    }
                    if(is_string($otherSubject))  {
                        if(strcmp($s, $otherSubject) !=0)  {
                            $otherSubject = true;
                        }
                    }
                    ...
                    //check if all the properties exist in the database
                    $result = $this->sqlQuery('select * from ef_stmt where p = \''.$p.'\' and o = \''.$oValue.'\'');
                    if(!$result)  {
                        $exists = false;
                    }
        ...
        //set released to N when version is incremented
        if($wikipage && $released && $exists  && is_string($otherSubject))  {
            //loop through all the queries
            foreach($insertArray as &$query)  {
                $string = $query;
                $position = strpos($query, 'http://localhost/ontowiki/view/released');
                if($position !== false)  { //if released property exists
                    $length = strlen('http://localhost/ontowiki/view/released');
                    $positionObject = $position+$length+2; //find the position of the objectValue
                    $commaPosition = strpos($query, "',", $positionObject); //find position of the end of the objectValue
                    $firstPart = substr($query, 0, $positionObject);
                    $secondPart = substr($query, $commaPosition, 9999);
                    $query = $firstPart.'\'N'.$secondPart; //change it to N
                }
            }
        }

        //add version information when there is a wikipage class and a label property and not at the creation of knowledge base
        if($wikipage && $label && is_string($otherSubject))  {
            //loop through all the queries
            foreach($insertArray as &$query)  {
                $versionString = ' - Version '; //string used for version information
                $string = $query;
                $position = strpos($query, "http://www.w3.org/2000/01/rdf-schema#label"); //find label propery
                if($position !== false)  { //if label propery exists
                    $length = strlen("http://www.w3.org/2000/01/rdf-schema#label");
                    $positionObject = $position+$length+2; //find the position of the objectValue
                    $commaPosition = strpos($query, "',", $positionObject); //find position of the end of the objectValue
                    $firstPart = substr($query, 0, $commaPosition);
                    if($exists)  { //if another version of the object exists
                        $positionVersion = strpos($firstPart, $versionString); //find position of appended version information
                        if($positionVersion !== false)  { //if version information has been found
                            $new = substr($firstPart, 0, $positionVersion);  //remove version information
                            $versionNumber = $firstPart[$positionVersion+strlen($versionString)];  //find old version number
                            $newVersion = $versionNumber + 1;  //increment version number
                            $result = $new.$versionString.$newVersion; //add new version information
                        } else  {  //no version information has been found - reset to version 1
                            $result = $firstPart.' - Version 1';
                        }
                    } else  {  //no other version exists
                        $result = $firstPart.' - Version 1';
                        //set released flag to N
                        $sqlString = '('.$graphId.',\''.$s.'\',\'http://localhost/ontowiki/view/released\', \'N\',\N,\N,\N,0,2,\'\',\'\',\N)';
                        $insertArray[] = $sqlString;
                        $counter++;
                    }
                    //rebuild query
                    ...
                    $result .= $secondPart;
                    $query = $result;
                } else  { //do nothing if the label propery does not exist
                   $query = $string;
                }
            }
        }
        //set released of wikipage to N at creation base and set label
        if(!is_string($otherSubject)  && sizeof($wikipageArray) > 0)  {
            foreach($wikipageArray as $wikipage)  {
                $position = strpos($wikipage, "#");
                $wikipageName = substr($wikipage, $position+1, 999);
                $releasedExists = false;
                $labelExists = false;
                foreach($insertArray as &$query)  {
                    $position = strpos($query, $wikipage);
                    if($position !== false)  {
                        $position = strpos($query, "http://www.w3.org/2000/01/rdf-schema#label"); //find label propery
                        $released = strpos($query, "http://localhost/ontowiki/view/released"); //find released property
                        if($released !== false)  {
                            $releasedExists = true;
                        }
                        if($position !== false)  { //if label propery exists
                            $labelExists = true;
                            $version = strpos($query, "- Version ");
                                if($version === false)  {
                                    $length = strlen("http://www.w3.org/2000/01/rdf-schema#label");
                                    $positionObject = $position+$length+2; //find the position of the objectValue
                                    $commaPosition = strpos($query, "',", $positionObject); //find position of the end of the objectValue
            ...
            if(!$releasedExists)  {
                $sqlString = '('.$graphId.',\''.$wikipage.'\',\'http://localhost/ontowiki/view/released\', \'N\',\N,\N,\N,0,2,\'\',\'\',\N)';
                $insertArray[] = $sqlString;
                $counter++;
            }
            if(!$labelExists)  {
                $sqlString = '('.$graphId.',\''.$wikipage.'\',\'http://www.w3.org/2000/01/rdf-schema#label\', \''.$wikipageName.' - Version 1\',\N,\N,\N,0,2,\'\',\'\',\N)';
                $insertArray[] = $sqlString;
                $counter++;
            }
        }
            ...
            foreach ($statementsArray as $subject => $predicatesArray) {
                foreach ($predicatesArray as $predicate => $objectsArray) {
                    foreach ($objectsArray as $object) {
                        ...
                        $whereString .= 'AND s = \'' . $subject . '\' ';
                        $whereString .= 'AND p = \'' . $predicate . '\' ';
                        if(!preg_match('/content/',$predicate))
...
// TODO dedicated exception
    ...
    private function _createTablesMysql()
    {
        ...
        //Create table ef_annotation if not existing
        $sqlsrv='CREATE TABLE IF NOT EXISTS `ef_annotation` (
				  `ID` int(11) NOT NULL AUTO_INCREMENT,
				  `instanceURI` varchar(500) NOT NULL,
				  `phrase` varchar(60) NOT NULL,
				  `conceptClass` varchar(220) NOT NULL,
				  PRIMARY KEY (`ID`)
					)';
        ...
        $sql = 'CREATE TABLE IF NOT EXISTS ef_stmt (
            	    ...
            	    o		TEXT(65000) COLLATE utf8_bin NOT NULL,     # object or object hash
            	    ...
            	    UNIQUE  unique_stmt (g, s, p, o(160), st, ot, ol, od),
            	    INDEX   idx_g_p_o_ot (g, p, o(160), ot),
            	    INDEX   idx_g_o_ot (g, o(160), ot)
