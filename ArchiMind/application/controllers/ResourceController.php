...
class ResourceController extends OntoWiki_Controller_Base
{
    ...
    {   
        ...
        if ($this->_owApp->mode == '')
        {
       		//first time sessionvar get is empty -> default is user mode
	        $this->_owApp->mode = 'user';
    	}
        $mode 		= $this->_owApp->mode;
		
        
        //check if it is a Wikipage instance
        $query = 'select * from ef_stmt where s = \''.$resource.'\' and p = \'http://www.w3.org/1999/02/22-rdf-syntax-ns#type\' and o = \'http://www.semanticweb.org/ontologies/2010/10/Ontology1288790966584.owl#WikiPage\'';
        ...
        //check if document is released or not
        if($wikipage)  {
            $query = 'select * from ef_stmt where s = \''.$resource.'\' and p = \'http://localhost/ontowiki/view/released\'';
            $result = $store->sqlQuery($query);
            $released = false;
            if($result)  {
                if(strcmp($result[0]['o'],'Y') == 0)  {
                    $released = true;
                }
            }
        }
        ...
        if ($this->_request->getParam('action') == 'properties' && $graph->isEditable() &&
          ...
          // TODO: check acl
            ...
            if($wikipage)  {
                if($released)  {
                    $toolbar->appendButton(OntoWiki_Toolbar::DELETE, array(
                        'name' => 'Unrelease page',
                        'url' => 'javascript:unrelease(\''.$this->view->resourceUri.'\', \''.$graph.'\');'
                    ));
                }  else  { //not released
                    $toolbar->appendButton(OntoWiki_Toolbar::ADD, array(
                        'name' => 'Release page',
                        'url' => 'javascript:release(\''.$this->view->resourceUri.'\', \''.$graph.'\');'
                    ));
                }
                $toolbar->appendButton(OntoWiki_Toolbar::ADD, array(
                    'name' => 'Increment version',
                    'url' => 'javascript:incrementVersion(\''.$this->view->resourceUri.'\', \''.$graph.'\');'
                ));
                $toolbar->appendButton(OntoWiki_Toolbar::METADATA, array(
                'name' => 'Edit metadata'
            ));
            }
            ...
            $toolbar->appendButton(OntoWiki_Toolbar::SEPARATOR)
					...
					//klaas
		             	$toolbar->appendButton(OntoWiki_Toolbar::USER, array(
		                    'name' => 'Switch to user mode',
		                    'url' => 'javascript:switchToUserMode();'
		                ));
		            }
		            else
		            {
		             	$toolbar->appendButton(OntoWiki_Toolbar::WIKIPAGE_OVERVIEW, array(
		                    'name' => 'Wikipages overview',
		                    'url' => $this->_config->urlBase . 'list?init&instancesconfig=%7B%22filter%22%3A%5B%7B%22mode%22%3A%22cnav%22%2C%22cnav%22%3A%7B%22state%22%3A%7B%22limit%22%3A%2225%22%2C%22path%22%3A%5B%5D%2C%22showEmpty%22%3Atrue%2C%22showImplicit%22%3Atrue%2C%22lastEvent%22%3A%22init%22%7D%2C%22config%22%3A%7B%22name%22%3A%22Classes%22%2C%22titleMode%22%3A%22titleHelper%22%2C%22hierarchyTypes%22%3A%5B%22http%3A%5C%2F%5C%2Fwww.w3.org%5C%2F2002%5C%2F07%5C%2Fowl%23Class%22%2C%22http%3A%5C%2F%5C%2Fwww.w3.org%5C%2F2000%5C%2F01%5C%2Frdf-schema%23Class%22%5D%2C%22hierarchyRelations%22%3A%7B%22in%22%3A%5B%22http%3A%5C%2F%5C%2Fwww.w3.org%5C%2F2000%5C%2F01%5C%2Frdf-schema%23subClassOf%22%5D%7D%2C%22instanceRelation%22%3A%7B%22out%22%3A%5B%22http%3A%5C%2F%5C%2Fwww.w3.org%5C%2F1999%5C%2F02%5C%2F22-rdf-syntax-ns%23type%22%5D%7D%2C%22hiddenNS%22%3A%5B%22http%3A%5C%2F%5C%2Fwww.w3.org%5C%2F1999%5C%2F02%5C%2F22-rdf-syntax-ns%23%22%2C%22http%3A%5C%2F%5C%2Fwww.w3.org%5C%2F2000%5C%2F01%5C%2Frdf-schema%23%22%2C%22http%3A%5C%2F%5C%2Fwww.w3.org%5C%2F2002%5C%2F07%5C%2Fowl%23%22%5D%2C%22hiddenRelation%22%3A%5B%22http%3A%5C%2F%5C%2Fns.ontowiki.net%5C%2FSysOnt%5C%2Fhidden%22%5D%2C%22showImplicitElements%22%3A%221%22%2C%22showEmptyElements%22%3A%221%22%2C%22showCounts%22%3A%22%22%2C%22checkSub%22%3A%221%22%2C%22hideDefaultHierarchy%22%3A%22%22%7D%7D%2C%22uri%22%3A%22http%3A%5C%2F%5C%2Fwww.semanticweb.org%5C%2Fontologies%5C%2F2010%5C%2F10%5C%2FOntology1288790966584.owl%23WikiPage%22%2C%22action%22%3A%22add%22%7D%5D%7D'
		                ));
		                
		             	$toolbar->appendButton(OntoWiki_Toolbar::USER, array(
		                    'name' => 'Switch to input mode',
		                    'url' => 'javascript:disableUserMode();'
		                ));
