...
function navigationLoadnoJSON (navEvent, setupp) {
    if (typeof setupp == 'undefined') {
        alert('error: No navigation setup given, but navigationLoad requested');
        return;
    }
    ...
    // preparation of a callback function
    var cbAfterLoad = function(){
        $.post(navigationExploreUrl, {setup: setupp},
            function (data) {
                //alert(data);
                navigationContainer.empty();
                navigationContainer.append(data);
...
function navigationLoadTwo (navEvent, setupp) {
    if (typeof setupp == 'undefined') {
        alert('error: No navigation setup given, but navigationLoad requested');
        return;
    }
    ...
    var cbAfterLoad = function(){
        $.post(navigationExploreUrl, {setup: setupp},
            function (data) {        		       	                
                ...
                navigationContainer.append(data);
                // remove the processing status
                navigationContainer.removeClass('is-processing');          
                switch (navEvent) {
                    case 'more':
                        navigationMore.remove();
                        // remove the processing status
    ...
    navigationContainer = $('#navigation-content2'); 
    
    // first we set the processing status
    navigationContainer.addClass('is-processing');
    navigationContainer.css('overflow', 'hidden');
