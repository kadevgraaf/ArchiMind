...
function toggleExpansion(event) {
    ...
    if (target.hasClass('expand')) {
        ...
        if (target.parent().children('.expansion').length) {
	        Diaglog.logSearch(target.next().attr('typeof') ? 'Expand listed instance' : 'Expand instance properties', $.trim(target.next().html().substr(0, target.next().html().indexOf('<span class'))), target.next().attr('rel').substr(1 + target.next().attr('rel').indexOf(':')), '', target.next().attr('typeof') ? target.next().attr('typeof') : target.next().attr('resource'), true);
            ...
            $.ajax({
                ...
                success:  function(content) {
					...
					//alert('test' + $.trim(target.next().html().substr(0, target.next().html().indexOf('<span class'))) + 'test2' + target.next().attr('typeof'));
					//alert("test" + target.next().attr('resource').substr(0,target.next().attr('resource').indexOf(':')));
					Diaglog.logSearch(target.next().attr('typeof') ? 'Expand listed instance' : 'Expand instance properties', $.trim(target.next().html().substr(0, target.next().html().indexOf('<span class'))), target.next().attr('rel').substr(1 + target.next().attr('rel').indexOf(':')), '', target.next().attr('typeof') ? target.next().attr('typeof') : target.next().attr('resource'), true);
					Highlight.getAnnotations(resourceUri);
...
function switchToUserMode()  {
	var setusermodeUrl = urlBase + 'usermode/switchtousermode';
...
function disableUserMode() {
	...
	var disableusermodeUrl = urlBase + 'usermode/disableusermode';
			$.post(disableusermodeUrl, {},
		            function (data) {
						document.location.reload(true);
			});					
}
...
function getMode() {
	...
	var getmodeUrl = urlBase + 'usermode/getmode';
			$.post(getmodeUrl, {},
		            function (data) {
						alert(data);
						return(data);
     ...
     //archimind_mode/set_mode.php
     /*
   				$.ajax({
				  type: 'POST',
				  url: urlBase + 'libraries/archimind_mode/set_mode.php',
				  async: false,
				  data: ({mode : 'user'}),
				  success: function(data) {alert('data1:' + data);}
				});    	   
				  ...
				  async: false,
				  data: ({mode : 'user'}),
				  success: function(data) {alert('data2:' + data);}
				});    	   	
...
//release a document
function release(resource, graph)  {
   var updateEndpoint = urlBase + 'update';
   var insert = {};
   var remove = {};

   insert[resource] = {'http://localhost/ontowiki/view/released': [{'type': 'literal', 'value': 'Y'}]};
   remove[resource] = {'http://localhost/ontowiki/view/released': [{'type': 'literal', 'value': 'N'}]};
...
//unrelease a document
function unrelease(resource, graph)  {
   var updateEndpoint = urlBase + 'update';

   var insert = {};
   var remove = {};

   insert[resource] = {'http://localhost/ontowiki/view/released': [{'type': 'literal', 'value': 'N'}]};
   remove[resource] = {'http://localhost/ontowiki/view/released': [{'type': 'literal', 'value': 'Y'}]};
...
//increment version number
 function incrementVersion(resource, graph)  {
    var serviceUri = urlBase + 'service/rdfauthorinit';
    ...
    var remove = {};
    var counter = 0;
    var query = 'SELECT DISTINCT ?predicate ?object where { <'+resource+'> ?predicate ?object .}';
    removeResourceMenus();
   ...
   function(data)  {
       $.each(data.results.bindings, function(i, triple) {
            predicates[counter] = triple.predicate.value;
            if(triple.predicate.value == "http://www.w3.org/2000/01/rdf-schema#label")  {
                label = counter;
            }
            if(triple.object.value == 'http://www.semanticweb.org/ontologies/2010/10/Ontology1288790966584.owl#WikiPage')  {
                documentType = counter;
            }
            if(triple.predicate.value == 'http://localhost/ontowiki/view/released')  {
                objects[counter] = 'N';
            }  else  {
                objects[counter] = triple.object.value;
            }
            objectTypes[counter] = triple.object.type;
            counter++;
    });
        ...
        $.getJSON(serviceUri, {
           ...
           uri: resource
            ...
            // grab first object key
            for (var subjectUri in data) {break;};
            var newSubject = urlBase+'view/'+subjectUri;
            insert[newSubject] = {};
            insert[newSubject][predicates[label]] = [{'type': objectTypes[label], 'value': objects[label]}];
            insert[newSubject][predicates[documentType]] = [{'type': objectTypes[documentType], 'value': objects[documentType]}];
            $.getJSON(updateEndpoint,{
                        'insert':  $.toJSON(insert),
                        'delete': $.toJSON(remove),
                        'named-graph-uri': graph
            ...
            for(var i = 0; i < predicates.length; i++)  {
                insert[newSubject] = {};
                if(i != label && i != documentType)  {
                    insert[newSubject][predicates[i]] = [{'type': objectTypes[i], 'value': objects[i]}];
                    $.getJSON(updateEndpoint,{
                            'insert':  $.toJSON(insert),
                            'delete': $.toJSON(remove),
                            'named-graph-uri': graph
           ...
           window.setTimeout(function() {
              alert('Version succesfully incremented!')
           }, 1000);
        })
   });
 }
